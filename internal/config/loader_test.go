...
func TestLoad_RuntimeValidationError(t *testing.T) {
	clearTestEnv(t)
	resetTestFlags(t)

	// Trigger applyRuntimeValidation error: cert CN prefix enabled with nonexistent cert
	t.Setenv("MQTT_USE_CERT_CN_PREFIX", "true")
	t.Setenv("MQTT_CLIENT_CERT", "/nonexistent/cert.pem")

	_, err := Load()
	if err == nil {
		t.Error("Load() error = nil; want runtime validation error for missing cert")
	}
}
...
func clearTestEnv(t *testing.T) {
	...
	for _, v := range envVars {
		...
		// preventing environment corruption across tests.
		t.Setenv(v, "")
		// After t.Setenv (which calls os.Setenv), unset the var so Load()
		// sees it as absent rather than empty-string.
		if err := os.Unsetenv(v); err != nil {
			t.Fatalf("failed to unsetenv %s: %v", v, err)
		}
	}
}
